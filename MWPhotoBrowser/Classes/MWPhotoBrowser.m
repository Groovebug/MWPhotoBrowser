...
- (void)performLayout {
    ...
    // Toolbar
    if (false) {//numberOfPhotos > 1 || _displayActionButton) {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        UIBarButtonItem *doneButton = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Done", nil) style:UIBarButtonItemStyleBordered target:self action:@selector(doneButtonPressed:)] autorelease];
        ...
        NSMutableDictionary *attributes = bar.titleTextAttributes.mutableCopy;
        attributes[UITextAttributeTextColor] = [UIColor whiteColor];
        bar.titleTextAttributes = attributes;
        UIImage *img = [self translucentImage];
        [bar setBackgroundImage:img forBarMetrics:UIBarMetricsDefault];
        [bar setBackgroundImage:img forBarMetrics:UIBarMetricsLandscapePhone];
    } else {
        // We're not first so don't do anything
    }
...
- (void)viewWillLayoutSubviews {
	...
	for (UIView<MWPhotoPage> * page in _visiblePages) {
...
- (void)tilePages {
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            ...
            // Add new page
            id<MWPhoto> photo = [self photoAtIndex:index];
			UIView<MWPhotoPage> *page = [self dequeueRecycledPageForPhoto:photo];
			if (!page) {
                if (photo.PDFURL) {
                    page = [[[MWPDFPage alloc] initWithPhotoBrowser:self] autorelease];
                } else {
...
- (UIView<MWPhotoPage> *)pageDisplayedAtIndex:(NSUInteger)index {
	UIView<MWPhotoPage> *thePage = nil;
	for (UIView<MWPhotoPage> *page in _visiblePages) {
...
- (UIView<MWPhotoPage> *)dequeueRecycledPageForPhoto:(id<MWPhoto>)photo {
	UIView<MWPhotoPage> *page = [[_recycledPages objectsPassingTest:^BOOL(id obj, BOOL *stop) {
        id<MWPhotoPage> page = obj;
        return page.supportsPDF == (photo.PDFURL!=nil);
    }] anyObject];
	if (page) {
